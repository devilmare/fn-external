	...
	public:
		...
		__forceinline std::vector < type > get_itter()
		{
			...
			try {
				...
				driver.read_memory(reinterpret_cast <std::uintptr_t> (this->data), buffer.data(), sizeof(type) * this->count);
		...
		__forceinline type operator[](std::uintptr_t idx) const
		{
			...
			return driver.read<type>(std::bit_cast<std::uintptr_t>(this->data) + (idx * sizeof(type)));
		}
